...
# copy of home.py file that does not require server interaction
...
class Home(ttk.Toplevel):
    ...
    def create_password_field(self):
        ...
        result = [
            {"url": "example.com", "username": "user1", "password": "password1"},
            {"url": "example.net", "username": "user2", "password": "password2"}
        ]
    ...
    def create_generate_password_field(self):
        ...
        scale = ttk.Scale(password_generate_frame, from_=8, to=21, length=100, variable=pass_length, bootstyle='PRIMARY', value=8, command=self.accept_whole_number_only)
        scale.pack(side='top', pady=10)
        pass_len_label = ttk.Label(master=password_generate_frame, bootstyle='PRIMARY', text='8 characters')
        pass_len_label.pack(side='top', pady=10)
